...
const defaultStorageDir = path.join(os.homedir(), 'GatiApp');
const appSettingsPath = path.join(__dirname, 'app-settings.json');

let storagePath = defaultStorageDir;
let pathData = path.join(storagePath, 'data.json');
let pathCatalogo = path.join(storagePath, 'catalogo_regalos.json');

function ensureStorageDirectory(dirPath) {
    fs.mkdirSync(dirPath, { recursive: true });
}

function guardarConfiguracionAlmacenamiento() {
    try {
        ensureStorageDirectory(path.dirname(appSettingsPath));
        fs.writeFileSync(appSettingsPath, JSON.stringify({ storagePath }, null, 4));
    } catch (err) {
        console.warn('No se pudo guardar la configuración de almacenamiento.', err);
    }
}

function actualizarRutaDatos(nuevaRuta) {
    const rutaBase = path.resolve(nuevaRuta || defaultStorageDir);
    storagePath = rutaBase;
    pathData = path.join(rutaBase, 'data.json');
    pathCatalogo = path.join(rutaBase, 'catalogo_regalos.json');
    ensureStorageDirectory(rutaBase);
    if (typeof configGlobal !== 'undefined') {
        configGlobal.storagePath = rutaBase;
    }
    guardarConfiguracionAlmacenamiento();
}
...
// app.get('/vista_configuracion', (req, res) => res.sendFile(path.join(__dirname, 'vistas', 'vista_configuracion.html')));
...
let configGlobal = {
    username: "", historial: [], storagePath: storagePath,
...
if (fs.existsSync(appSettingsPath)) {
    try {
        ...
        if (appSettings?.storagePath) {
            actualizarRutaDatos(appSettings.storagePath);
        }
    } catch (e) { }
}

const legacyDataPath = path.join(__dirname, 'data.json');
const legacyCatalogoPath = path.join(__dirname, 'catalogo_regalos.json');
...
if (fs.existsSync(pathData)) {
    try {
        ...
        configGlobal.storagePath = storagePath;
    } catch (e) { }
} else if (fs.existsSync(legacyDataPath)) {
    try {
        let guardado = JSON.parse(fs.readFileSync(legacyDataPath, 'utf8'));
        configGlobal = { ...configGlobal, ...guardado };
        if (!configGlobal.rachaVersus) configGlobal.rachaVersus = { salvadas: {}, reinicios: {}, showName: true, showCount: true, showCoins: true };
        if (!configGlobal.bolita) configGlobal.bolita = { multiplicador: 2, chatWord: "globos, jugar", chatGlobos: 1, chatCooldown: 60, likesMeta: 50, likesGlobos: 1, followGlobos: 5, followCooldown: 300, allowFree: true, quiereMeGlobos: 60 };
        if (!configGlobal.metaLikes) configGlobal.metaLikes = { active: false, firstGoal: 0, step: 20000, prefixText: "A los", actionText: "REINICIO", currentGoal: 20000, style: { fontSize: 45, color: "#ffffff", shadowColor: "#ff003c", fontFamily: "'Luckiest Guy', cursive" } };
        ...
        if (!configGlobal.topVIP.displayLimit) configGlobal.topVIP.displayLimit = 2;
        if (configGlobal.topVIP.mirrorMode === undefined) configGlobal.topVIP.mirrorMode = false;
        actualizarRutaDatos(storagePath);
        fs.copyFileSync(legacyDataPath, pathData);
        if (fs.existsSync(legacyCatalogoPath)) fs.copyFileSync(legacyCatalogoPath, pathCatalogo);
    } catch (e) { }
}
...
const proxySocket = createSocketClient(GATIPROXY_URL, {
...
let proxyActiveUsername = null;
let lastRequestedUsername = null;
...
function isSameGiftId(a, b) {
    return String(a) === String(b) || Number(a) === Number(b);
}

function getEventUser(data) {
    return data?.user?.displayId || data?.user?.id || data?.user?.secUid || 'anon';
}

function getEventDisplayName(data, fallbackUser) {
    const cleanName = (data?.user?.nickname || data?.user?.displayId || fallbackUser || 'ANONIMO')
        .replace(/[^a-zA-Z0-9\sÁÉÍÓÚáéíóúÑñ]/g, '')
        .trim() || fallbackUser || 'ANONIMO';
    return cleanName.length > 12 ? `${cleanName.substring(0, 12)}...` : cleanName;
}

function getEventAvatar(data) {
    return (
...
function desconectarTikTok() {
    ...
    if (proxyReady && proxyActiveUsername) {
        proxySocket.emit('proxy:disconnect', proxyActiveUsername, () => { });
    }

    proxyActiveUsername = null;
...
function conectarTikTok(usuario) {
    const userLimpio = normalizeUsername(usuario);
    if (!userLimpio) { io.emit('estado_conexion', { estado: 'desconectado', msg: '⚠️ Ingresa un usuario para conectar' }); return; }
    ...
    if (proxyReady && proxyActiveUsername && proxyActiveUsername !== userLimpio) {
        proxySocket.emit('proxy:disconnect', proxyActiveUsername, () => { });
    }

    desconexionIntencional = false;
    lastRequestedUsername = userLimpio;
    io.emit('estado_conexion', { estado: 'conectando', msg: `🟡 Conectando a @${userLimpio}...` });
    if (!proxyReady) {
        io.emit('estado_conexion', { estado: 'conectando', msg: '🟡 Esperando conexión con Gatiproxy...' });
        return;
    }

    proxySocket.emit('proxy:connect', userLimpio, (response) => {
        if (!response?.ok) {
            io.emit('estado_conexion', { estado: 'error', msg: `❌ Error: ${response?.error || 'No se pudo conectar'}` });
            return;
        }

        if (response?.session?.status === 'error') {
            io.emit('estado_conexion', { estado: 'error', msg: `❌ Error: ${response?.session?.lastError || 'No en Live o no existe'}` });
            return;
        }

        proxyActiveUsername = response?.session?.username || userLimpio;
        io.emit('estado_conexion', { estado: 'conectado', msg: `🟢 Conectado a @${proxyActiveUsername} | 📡 Radar Activo` });

        if (!configGlobal.historial.includes(proxyActiveUsername)) {
            configGlobal.historial.unshift(proxyActiveUsername);
...
function processLikeEvent(data) {
    const totalTikTok = parseInt(data.total, 10);
    const batchLikes = parseInt(data.count, 10) || 1;
    ...
    if (configGlobal.metaLikes && configGlobal.metaLikes.active) {
        const meta = configGlobal.metaLikes;
        let cGoal = parseInt(meta.currentGoal, 10) || 0;
        const mStep = parseInt(meta.step, 10) || 20000;
    ...
    const cleanName = getEventDisplayName(data, user);
    const avatarUrl = getEventAvatar(data);
...
function processGiftEvent(data) {
    ...
    const eventUser = getEventUser(data);
    const huellaRegalo = msgId || `${eventUser}_${data?.common?.createTime || Date.now()}`;
    ...
    let unitPrice = parseInt(data?.gift?.diamondCount || data.diamondCount || 0, 10) || 0;
    const giftId = data.giftId;
    const giftName = data?.gift?.name || data.giftName || "Regalo";
    const knownGift = catalogoGlobal.find(g => isSameGiftId(g.id, giftId));

    if (knownGift) {
        unitPrice = knownGift.diamonds;
    } else {
        const nuevoRegalo = { id: giftId, name: giftName, diamonds: unitPrice };
        catalogoGlobal.push(nuevoRegalo);
    ...
    const totalCoins = unitPrice * repeatCount;
    ...
    if (!topDonators[user]) topDonators[user] = { monedas: 0, avatar: avatarUrl, displayName: cleanName };
        ...
        topDonators[user].avatar = avatarUrl;
        topDonators[user].displayName = cleanName;
    }
    ...
    if (!configGlobal.racha.topRound[user]) configGlobal.racha.topRound[user] = { monedas: 0, avatar: avatarUrl, displayName: cleanName };
        ...
        configGlobal.racha.topRound[user].avatar = avatarUrl;
        configGlobal.racha.topRound[user].displayName = cleanName;
    }
    configGlobal.racha.topRound[user].monedas += totalCoins;
    io.emit('racha_data_update', configGlobal.racha);

    const isSalvar = configGlobal.equipo1.regalos.some(r => isSameGiftId(r.id, giftId));
    const isReiniciar = configGlobal.equipo2.regalos.some(r => isSameGiftId(r.id, giftId));
    ...
    if (isSalvar) {
        ...
        teamSalvar.donators[user].coins += totalCoins;
        ...
        teamReiniciar.donators[user].coins += totalCoins;
        triggeredTeam = 'reiniciar';
    }

    if (triggeredTeam) {
        emitSalvarUpdate(io);
        io.emit('poder_salvar', { side: triggeredTeam, amount: totalCoins });
    }
}

proxySocket.on('connect', () => {
    proxyReady = true;
    if (lastRequestedUsername) conectarTikTok(lastRequestedUsername);
});

proxySocket.on('connect_error', (error) => {
    proxyReady = false;
    io.emit('estado_conexion', { estado: 'error', msg: `❌ Error de Gatiproxy: ${error.message}` });
});

proxySocket.on('disconnect', () => {
    proxyReady = false;
    if (!desconexionIntencional) {
        io.emit('estado_conexion', { estado: 'conectando', msg: '🟡 Conexión con Gatiproxy perdida. Reintentando...' });
    }
});

proxySocket.on('proxy:update', (message) => {
    const type = message?.type;
    const payload = message?.payload || {};
    const username = payload.username;

    if (proxyActiveUsername && username && username !== proxyActiveUsername) return;

    if (type === 'session-connected') {
        proxyActiveUsername = payload.username || proxyActiveUsername;
        return;
    }

    if (type === 'session-event') {
        if (payload.event === 'like') {
            processLikeEvent(payload.data || {});
            return;
        }
        if (payload.event === 'gift') {
            processGiftEvent(payload.data || {});
            return;
        }
        if (payload.event === 'streamEnd') {
            desconexionIntencional = true;
            io.emit('estado_conexion', { estado: 'offline', msg: '⬛ El LIVE ha finalizado' });
        }
        return;
    }

    if (type === 'session-error') {
        io.emit('estado_conexion', { estado: 'error', msg: `❌ Error: ${payload.error || 'Error de sesión'}` });
        return;
    }

    if (type === 'session-disconnected') {
        if (!desconexionIntencional && proxyActiveUsername) {
            io.emit('estado_conexion', { estado: 'conectando', msg: '🟡 Conexión perdida. Auto-reconectando...' });
            reconnectionTimer = setTimeout(() => {
                reconnectionTimer = null;
                conectarTikTok(proxyActiveUsername);
            }, 3000);
        }
    }
});
...
io.on('connection', (socket) => {
    ...
    socket.on('guardar_config', (nuevaConfig) => {
        ...
        nuevaConfig.storagePath = storagePath;
        configGlobal = nuevaConfig; guardarEnArchivo(); io.emit('config_actual', configGlobal); emitSalvarUpdate(io);
    });

    socket.on('guardar_ruta_datos', (ruta) => {
        const rutaLimpia = String(ruta || '').trim();
        actualizarRutaDatos(rutaLimpia || defaultStorageDir);
        guardarEnArchivo();
        io.emit('config_actual', configGlobal);
        io.emit('ruta_datos_actualizada', { storagePath });
    });
    ...
    socket.on('force_fetch_likes', async () => {
        if (!proxyActiveUsername) return;

        try {
            const response = await fetch(`${GATIPROXY_URL}/sessions/${encodeURIComponent(proxyActiveUsername)}/room-info`);
            if (!response.ok) return;

            const json = await response.json();
            const roomInfo = json?.roomInfo || {};

            const fetchedLikes =
                parseInt(roomInfo?.like_count, 10) ||
                parseInt(roomInfo?.stats?.likeCount, 10) ||
                parseInt(roomInfo?.stats?.totalLikeCount, 10) ||
                parseInt(roomInfo?.owner?.stats?.heartCount, 10) ||
                0;

            if (!isNaN(fetchedLikes) && fetchedLikes > 0) {
                currentTotalLikes = fetchedLikes;
                io.emit('sync_likes_actuales', currentTotalLikes);
            }
        } catch (error) {
            console.log("No se pudo extraer la información de likes desde Gatiproxy.", error);
        }
    });
...
const PORT = process.env.PORT || 23847;
http.listen(PORT, () => {
